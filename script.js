...
function renderMembers() {
    ...
    const mentorSection   = $('mentors-section');
    const mentorsGrid     = $('mentors-grid');
        ...
        if (mentorsGrid) {
            ...
            <div class="mentor-card rounded-2xl p-6 flex flex-col items-center text-center reveal group w-full sm:w-[calc(50%-1rem)] lg:w-[calc(33.333%-1.5rem)] max-w-[320px] mx-auto ${hasLink ? 'has-link cursor-pointer' : ''}" 
                ...
                <div class="w-32 h-32 sm:w-40 sm:h-40 mb-5 rounded-2xl overflow-hidden bg-gray-100 dark:bg-gray-800 relative shrink-0 shadow-inner border border-gray-200 dark:border-gray-700">
                    <img src="${escHtml(m.image || avatarUrl(m.name))}" alt="${escHtml(m.name)}" loading="lazy" decoding="async" class="absolute inset-0 w-full h-full object-cover object-top transition-transform duration-500 group-hover:scale-105" onerror="this.src='${avatarUrl(m.name)}'">
                </div>
                <div class="w-full">
    ...
    } else if (mentorSection) {
        mentorSection.classList.add('hidden');
    }

    setTimeout(() => qsa('#members .reveal').forEach(el => revealObserver.observe(el)), 50);
}
...
function openTeamModal(teamName) {
    ...
    const subtitleEl = $('team-modal-subtitle');
    const grid = $('team-modal-grid');
    const modal = $('team-modal');

    if (titleEl) titleEl.textContent = teamName;
    if (subtitleEl) subtitleEl.textContent = `${team.length} Member${team.length !== 1 ? 's' : ''}`;
    if (grid) grid.innerHTML = team.map((m, i) => generateMemberCardHtml(m, i, false)).join('');
    if (!modal) return;
 ...
 // --- Achievement Single-View Carousel + Hover Popup ---
    ...
    if (state.achievements.length === 0) {
        ...
        function openAchievementModal(i) {
            ...
            const titleEl = $('achievement-modal-title');
            const namesEl = $('achievement-modal-names');
            const dateEl = $('achievement-modal-date');
            if (titleEl) titleEl.textContent = a.title || '';
            if (namesEl) namesEl.innerHTML = a.names ? `<i class="fas fa-users mr-1.5"></i>${escHtml(a.names)}` : '';
            if (dateEl) {
            ...
            const descEl = $('achievement-modal-desc');
            if (descEl) descEl.textContent = a.description || '';

            const imgWrap = $('achievement-modal-image-wrap');
            const img = $('achievement-modal-image');
            if (a.image && img && imgWrap) {
                img.src = a.image;
                img.alt = a.title || '';
            ...
            const linkBtn = $('achievement-modal-link');
            if (linkBtn) {
                if (a.link) {
                    linkBtn.href = a.link;
...
async function handleContactSubmit(event) {
    ...
    const origTxt = btn ? btn.innerHTML : 'Sending…';
...
const mobileMenuToggle = $('mobile-menu-toggle');
    ...
    if (mobileMenuLinks.length) mobileMenuLinks.forEach(link => link.addEventListener('click', closeMobileMenu));
...
function showToast(message, type = 'success') {
    ...
    toastIcon.className = type === 'error' ? 'fas fa-times-circle text-red-400 text-xl' : 'fas fa-check-circle text-brand-orange text-xl';
